...
Begin VB.Form LotsLTp02a 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Lots Available By Part Number"
   ClientHeight    =   3360
   ClientLeft      =   2115
   ClientTop       =   1125
   ClientWidth     =   8085
   ...
   Begin VB.CommandButton cmdFnd 
      DownPicture     =   "LotsLTp02a.frx":07AE
      Height          =   315
      Left            =   5160
      Picture         =   "LotsLTp02a.frx":0AF0
      Style           =   1  'Graphical
      TabIndex        =   1
      TabStop         =   0   'False
...
Private Sub Form_Activate()
   ...
   If bOnLoad = 1 Then CreateTempTable
   bOnLoad = 0
   MouseCursor 0
   FillCombo
   cmbPrt = "ALL"
...
Private Sub PrintReport()
   ...
   sCustomReport = GetCustomReport("invlt02")
...
Private Sub SaveOptions()
   ...
   sOptions = Trim(optDet.Value) & chkIncludeLotComments.Value
   SaveSetting "Esi2000", "EsiInvc", "lt02", sOptions
...
Private Sub GetOptions()
   Dim sOptions As String
   On Error Resume Next
   sOptions = GetSetting("Esi2000", "EsiInvc", "lt02", sOptions) & "0000"
   'If Len(sOptions) > 0 Then optDet.Value = Val(sOptions)
   optDet.Value = Val(Mid(sOptions, 1, 1))
   Me.chkIncludeLotComments.Value = Val(Mid(sOptions, 2, 1))
End Sub
...
Private Sub FillReportTableh()
   ...
   ' Erase sLots
   sSql = "TRUNCATE TABLE EsReportLots01t"
   clsADOCon.ExecuteSQL sSql
   iTotalLots = 0
   ...
   bDataMaxed = 0
   If cmbPrt = "ALL" Then sPartNo = "" Else sPartNo = Compress(cmbPrt)
   sSql = "SELECT *,LOINUMBER,LOITYPE FROM LohdTable,LoitTable " _
          & "WHERE (LOTNUMBER=LOINUMBER AND LOIRECORD=(SELECT MAX(LOIRECORD) FROM LoitTable a WHERE " _
          & " LOTNUMBER= a.LOINUMBER)) AND (" _
          & "LOIPARTREF LIKE '" & sPartNo & "%' AND " _
          & "LOIADATE BETWEEN '" & sBegDate & " 00:00' AND '" _
          & sEndDate & " 23:59' AND LOTREMAININGQTY>" & Val(txtQty) _
          & " AND LOTLOCATION LIKE '" & Trim(txtLoc) & "%')" _
          & " AND LOTPARTREF NOT IN (select PARTREF from PartTable where (PAOBSOLETE = 1 or PAINACTIVE = 1))"
Debug.Print sSql

   bSqlRows = clsADOCon.GetDataSet(sSql, RdoLot, ES_KEYSET)
   If bSqlRows Then
      With RdoLot
         b = 1
         ...
         Do Until .EOF
            ...
            '                        If iTotalLots > 1000 Then
            '                            bDataMaxed = 1
            '                            Exit Do
            '                        End If
            lCOUNTER = lCOUNTER + 1
            sSql = "INSERT INTO EsReportLots01t (LOTREC," _
                   & "LOTID) VALUES(" & lCOUNTER _
                   & ",'" & Trim(!lotNumber) & "')"
            clsADOCon.ExecuteSQL sSql
            ...
            RdoRpt!LOTLOCATION = "" & Trim(!LOTLOCATION) 'PO?
            If !LOTPO > 0 Then
   ...
   Set RdoLot = Nothing
   Set RdoRpt = Nothing
   If bDataMaxed = 0 Then
      If b = 1 Then FillReportTabled
   Else
      MsgBox "This Report Has Reached It's 1000 Rows Of Data. The" & vbCr _
         & "Report Is Incomplete. Please Narrow Your Search Range.", _
...
Private Sub FillReportTabled()
   ...
   sSql = "SELECT * FROM EsReportLots01d WHERE " _
          & "LoiSysNumber=''"
   bSqlRows = clsADOCon.GetDataSet(sSql, RdoRpt, ES_KEYSET)
   sSql = "SELECT * FROM EsReportLots01t ORDER BY LOTREC"
   bSqlRows = clsADOCon.GetDataSet(sSql, RdoRow, ES_KEYSET)
   If bSqlRows Then
      Do Until RdoRow.EOF
...
'                  Select Case !LOITYPE
'                  Case 19
'                      If Len(Trim("" & !INREF2)) = 0 Then RdoRpt!LoiComments = "" & Trim(!LOICOMMENT) Else RdoRpt!LoiComments = "" & !INREF2
'                  Case Else
'                      RdoRpt!LoiComments = "" & Trim(!LOICOMMENT)
'                  End Select
...
Private Sub PrepareReport()
   ...
   FillReportTableh
   MouseCursor 0
   If bDataMaxed = 0 Then PrintReport
   prg1.Visible = False
   Exit Sub
...
'   sTableDef = "#" & Trim$(UCase(Format(Now, "ddd")) _
'               & Right(Compress(GetNextLotNumber()), 8))
'   sSql = "CREATE TABLE " & sTableDef & " (" _
'          & "LOTREC INT NULL DEFAULT(0)," _
'          & "LOTID CHAR(15) NULL DEFAULT(''))"
'   clsADOCon.ExecuteSQL sSql
'   If Err = 0 Then
'      sSql = "CREATE UNIQUE CLUSTERED INDEX LotRec ON " _
'             & sTableDef & "(LOTREC) WITH FILLFACTOR = 80"
'      clsADOCon.ExecuteSQL sSql
'   End If
