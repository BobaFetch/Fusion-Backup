...
Begin VB.Form MatlMMp02a 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Part Quantity Activity"
   ClientHeight    =   3405
   ClientLeft      =   2115
   ClientTop       =   1125
   ClientWidth     =   7500
   ...
   Begin VB.ComboBox cmbPrt 
      ...
      Width           =   3075
   End
   Begin VB.CheckBox cbGroupByLoc 
      Caption         =   "___"
      ForeColor       =   &H8000000F&
      Height          =   255
      Left            =   1680
...
Private Sub PrintReport()
   ...
   On Error GoTo DiaErr1
    ...
    aFormulaName.Add "GroupByLocation"
    aFormulaName.Add "ReportHeading"
    aFormulaName.Add "ReportSubHeading"
    
    aFormulaValue.Add CStr("'" & CStr(sFacility) & "'")
    aFormulaValue.Add CStr("'" & CStr(txtBeg) & "'")
    aFormulaValue.Add CStr("'" & CStr(txtEnd) & "'")
    aFormulaValue.Add CStr("'Requested By: " & CStr(sInitials) & "'")
    aFormulaValue.Add cbGroupByLoc.Value
    If optAct.Value = True Then aFormulaValue.Add CStr("'Part Quantity Activity (Actual Date)'") _
       Else aFormulaValue.Add CStr("'Part Quantity Activity (User Entered Date)'")
    If cbGroupByLoc.Value = 1 Then aFormulaValue.Add CStr("'(By Location)'") Else aFormulaValue.Add CStr("''")
   ...
   sCustomReport = GetCustomReport("admin02")
  ...
  If optAct.Value = True Then
      sSql = "{PartTable.PARTREF}='" & sPartNumber & "'" _
             ...
             & "to Date(" & sEndDate & ")"
   Else
'      sCustomReport = GetCustomReport("admin02p")
'      cCRViewer.SetReportFileName sCustomReport, sReportPath
'      cCRViewer.SetReportTitle = sCustomReport
      sSql = "{PartTable.PARTREF}='" & sPartNumber & "'" _
