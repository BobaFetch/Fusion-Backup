...
Begin VB.Form diaARe01d
   ...
   Begin Threed.SSRibbon cmdHlp
      ...
      BevelWidth = 0
      Outline = 0 'False
      PictureUp = "diaCsitm.frx":0000
      PictureDn = "diaCsitm.frx":0146
   End
   Begin VB.Label z1
      Alignment = 2 'Center
...
Public Function GetThisItem() As Byte
   ...
   RdoQry(1) = Val(lblItm)
   RdoQry(2) = lblRev
   bSqlRows = GetQuerySet(RdoItm, RdoQry)
   If bSqlRows Then
      With RdoItm
         lblItm = "" & Str(!ITNUMBER)
         lblRev = "" & Trim(!ITREV)
         ...
         'txtSdt = "" & Format(!ITSCHED, "mm/dd/yy")
         'txtRdt = "" & Format(!ITCUSTREQ, "mm/dd/yy")
         'txtDdt = "" & Format(!ITSCHEDDEL, "mm/dd/yy")
...
Private Sub txtPrc_LostFocus()
   ...
   txtPrc = Format(Abs(Val(txtPrc)), "####0.000")
   If bGoodPart Then
      If Val(txtQty) > 0 Then
         'cmbJmp.Enabled = True
         cmdAdd.Enabled = True
...
Private Sub txtQty_LostFocus()
   ...
   If Val(txtQty) = 0 Then txtQty = Format(cOldQty, "####0.000")
   cOldQty = Val(txtQty)
