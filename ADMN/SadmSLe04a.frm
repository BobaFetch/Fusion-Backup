...
Begin VB.Form SadmSLe04a 
   ...
   Begin VB.Label z1 
      ...
      Caption         =   "(Forces Use Of Fixed Terms In All Sections)"
      Height          =   255
      Index           =   3
      Left            =   1680
      TabIndex        =   8
...
Private Sub Form_Load()
   ...
   sSql = "SELECT TRMREF,TRMDESC,TRMCOMT FROM " _
          & "StrmTable WHERE TRMREF= ? "
...
Private Function GetTerms() As Byte
   ...
   bSqlRows = clsADOCon.GetQuerySet(RdoTrm, AdoQry, ES_KEYSET, True)
   If bSqlRows Then
      With RdoTrm
         cmbTrm = "" & Trim(!TRMREF)
         txtDsc = "" & Trim(!TRMDESC)
...
Private Sub AddTerms()
   ...
   sTerms = cmbTrm
   sMsg = sTerms & " Wasn't Found. Add The Shipping Terms?"
   bResponse = MsgBox(sMsg, ES_YESQUESTION, Caption)
   If bResponse = vbYes Then
      On Error GoTo DiaErr1
      sSql = "INSERT INTO StrmTable (TRMREF) " _
             & "VALUES('" & sTerms & "')"
      clsADOCon.ExecuteSQL sSql
      If clsADOCon.RowsAffected Then
         SysMsg "Terms Added.", True
         cmbTrm = sTerms
