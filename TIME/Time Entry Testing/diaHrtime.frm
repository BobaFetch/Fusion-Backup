...
'         & "order by case when len(TCSTART) < 6 then '' else substring(TCSTART,6,1)end, TCSTART"
      ...
      iMaxNumber = RdoCrd.RecordCount - 1
      ...
      iTotalPages = IIf(RdoCrd.RecordCount Mod LINES_PER_PAGE = 0, _
         RdoCrd.RecordCount / LINES_PER_PAGE + 1, _
         RdoCrd.RecordCount \ LINES_PER_PAGE + 1)
      
      If bSqlRows Then
         With RdoCrd
            Do Until .EOF
               iList = iList + 1
'               If iList > iMaxNumber Then
'                  MsgBox CStr(iList) & " > " & CStr(iMaxNumber)
'               End If
               If !TCRUNNO > 0 Then
                  vTimeCard(iList, TC_DI) = 1
               Else
...
Private Sub GetNextGroup()
   ...
   For iList = iIndex To iIndex + LINES_PER_PAGE - 1
      If (iList > UBound(vTimeCard)) Then
         txtMon(I) = ""
         txtRun(I) = ""
         txtOpn(I) = ""
         txtBeg(I) = "  :  "
...
'   For iList = 0 To LINES_PER_PAGE * MAX_PAGES - 1
   For iList = 0 To UBound(vTimeCard)
If iList = 101 Then
Debug.Print CStr(iList) & " of " & CStr(UBound(vTimeCard))
End If
