...
Begin VB.Form BookBKp04a 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Bookings By Division, Class And Code"
   ClientHeight    =   4050
   ClientLeft      =   2115
   ClientTop       =   1125
   ClientWidth     =   7260
...
Private Sub PrintReport()
   ...
   aFormulaValue.Add CStr("'Division" & CStr(cmbDiv & ", Class " & cmbCls & ",Code " & cmbCde & " " _
   ...
   cCRViewer.Init
   sCustomReport = GetCustomReport("slebk04")
   cCRViewer.SetReportFileName sCustomReport, sReportPath
   cCRViewer.SetReportTitle = sCustomReport
   cCRViewer.SetReportFormulaFields aFormulaName, aFormulaValue
   
   sSql = "{SohdTable.SOCUST} LIKE '" & sCust & "*' " _
...
Private Sub FormatControls()
   ...
   txtEnd = Format(ES_SYSDATE, "mm/dd/yyyy3")
   cmbDiv.AddItem "ALL"
...
Private Sub SaveOptions()
   ...
   'Save by Menu Option
   sDiv = Left(cmbDiv & Space(4), 4)
   sCust = Left(txtCst & Space(10), 10)
   sClas = Left(cmbCls & Space(4), 4)
   sCode = Left(cmbCde & Space(6), 6)
   sOptions = sDiv _
   ...
   SaveSetting "Esi2000", "EsiSale", "bk04a", sOptions
...
Private Sub GetOptions()
   Dim sOptions As String
   On Error Resume Next
   sOptions = GetSetting("Esi2000", "EsiSale", "bk04a", Trim(sOptions))
   If Len(sOptions) Then
      cmbDiv = Trim(Mid(sOptions, 1, 4))
      optDsc = Trim(Mid(sOptions, 5, 1))
      txtCst = Trim(Mid(sOptions, 6, 10))
      cmbCls = Trim(Mid(sOptions, 16, 4))
      cmbCde = Trim(Mid(sOptions, 20, 6))
