VERSION 5.00
Begin VB.Form BookBLp08b 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Customer Backlog Analysis"
   ClientHeight    =   9000
   ClientLeft      =   30
   ClientTop       =   330
   ClientWidth     =   8310
   Icon            =   "BookBLp08b.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.CommandButton optPrn 
      DownPicture     =   "BookBLp08b.frx":030A
      Height          =   415
      Left            =   7800
      Picture         =   "BookBLp08b.frx":0494
      Style           =   1  'Graphical
      TabIndex        =   1
      TabStop         =   0   'False
   ...
   Begin VB.Label z1 
      ...
      Left            =   2520
      TabIndex        =   8
      ToolTipText     =   "Uses Actual Shipment Date"
      Top             =   8880
      Visible         =   0   'False
      Width           =   612
   End
   ...
   Begin VB.Label lblThrough 
      ...
      Left            =   3600
      TabIndex        =   5
      ToolTipText     =   "Uses Scheduled Date"
      Top             =   840
      Width           =   732
   End
...
Private Sub Form_Load()
   ...
   For iRow = 0 To 19
      ...
      Sales(iRow).ToolTipText = "Total Backlog For This Customer"
   Next
   bOnLoad = 1
...
Private Sub CreateTempTable()
   ...
   sSql = "select DISTINCT SOCUST,CUNAME,ITSO FROM SohdTable,CustTable," _
          & "SoitTable WHERE (SONUMBER=ITSO  AND ITCANCELED=0 AND ITSCHED Between '" & sBeg & " 00:00' " _
          & "AND '" & sEnd & " 23:59' AND ITPSSHIPPED=0 AND SOCUST=CUREF) AND " _
          & "SOCUST LIKE '" & sCust & "%' ORDER BY SOCUST"
...
Private Sub CreateCustChart()
   ...
   For iRow = 0 To iTotalCusts
      sSql = "SELECT SOCUST,SONUMBER,SUM(ITQTY*ITDOLLARS)AS CustSales FROM " _
             ...
             & "ITSCHED BETWEEN '" & sBeg & " 00:00' AND '" & sEnd & " 23:59' " _
             & "AND ITPSSHIPPED=0 AND SOCUST='" & sCustomers(iRow, 0) & "') " _
             & "GROUP BY SOCUST,SONUMBER "
      bSqlRows = clsADOCon.GetDataSet(sSql, RdoChart, ES_FORWARD)
      If bSqlRows Then
         With RdoChart
            Do Until .EOF
   ...
   If bSqlRows Then
      With RdoChart
         ...
         ClearResultSet RdoChart
      End With
'      z1(1).Top = Customer(iTotalCusts).Top + 600
'      TotalSales.Top = Customer(iTotalCusts).Top + 600
      z1(1).Top = Customer(maxIndex).Top + 600
      TotalSales.Top = Customer(maxIndex).Top + 600
