VERSION 5.00
Begin VB.Form BookBLp05a 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Backlog By Customer"
   ClientHeight    =   5160
   ClientLeft      =   2115
   ClientTop       =   1125
   ClientWidth     =   7260
...
Private Sub PrintReport()
   ...
   cCRViewer.Init
   sCustomReport = GetCustomReport("slebl04")
   ...
   Next
   sSql = sSql & ")"
   sSql = sSql & " and {SoitTable.ITPSSHIPPED} = 0 and" _
               & " {SoitTable.ITCANCELED} = .000 and" _
               & " {SoitTable.ITPSNUMBER} = '' and " _
               & " {SoitTable.ITINVOICE} = .000"
...
Private Sub SaveOptions()
   ...
   SaveSetting "Esi2000", "EsiSale", "bL05", Trim(sOptions)
   sOptions = ""
   For b = 0 To 25
      sOptions = sOptions & Trim$(optTyp(b).Value)
   Next
   SaveSetting "Esi2000", "EsiSale", "bl05b", Trim(sOptions)
...
Private Sub GetOptions()
   ...
   On Error Resume Next
   sOptions = GetSetting("Esi2000", "EsiSale", "bL05", Trim(sOptions))
   ...
   On Error GoTo DiaErr1
   sOption2 = GetSetting("Esi2000", "EsiSale", "bl05b", Trim(sOption2))
