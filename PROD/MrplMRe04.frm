...
Begin VB.Form MrplMRe04 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Auto Release SC Status MO's (Old Version)"
   ClientHeight    =   10725
   ClientLeft      =   2115
   ClientTop       =   1125
   ClientWidth     =   15240
   ...
   Begin VB.CommandButton cmdFnd 
      DownPicture     =   "MrplMRe04.frx":0000
      Height          =   315
      Left            =   5160
      Picture         =   "MrplMRe04.frx":0342
      Style           =   1  'Graphical
      TabIndex        =   2
      TabStop         =   0   'False
...
'   sSql = "SELECT MRP_PARTREF,0 as RUNNO, 0 as RUNOPCUR, MRP_PARTNUM," & vbCrLf _
'             & "MRP_PARTQTYRQD, MRP_PARTDATERQD, MrplTable.MRP_ACTIONDATE, '' RUNSTATUS " & vbCrLf _
'         & " FROM MrplTable, PartTable,  (SELECT DISTINCT BMASSYPART, MRP_ACTIONDATE " & vbCrLf _
...
'            & "AND PartTable.PAMAKEBUY ='M'" & vbCrLf _
'            & "AND MrplTable.MRP_PARTREF = f.BMASSYPART" & vbCrLf _
'            & "AND MrplTable.MRP_ACTIONDATE = f.MRP_ACTIONDATE" & vbCrLf _
'         & " UNION SELECT DISTINCT RUNREF as MRP_PARTREF, RUNNO, RUNOPCUR, PARTNUM as MRP_PARTNUM, " & vbCrLf _
'                & " RUNQTY As MRP_PARTQTYRQD, RUNPKSTART As MRP_PARTDATERQD, RUNSCHED As MRP_ACTIONDATE, RUNSTATUS " & vbCrLf _
...
'            & " AND RUNPKSTART BETWEEN '" & sBDate & "' AND '" & sEDate & "' " & vbCrLf _
'            & " AND RUNREF = f.BMASSYPART AND RUNSCHED = f.MRP_ACTIONDATE " & vbCrLf _
'            & " order by MRP_PARTDATERQD"
   ...
   '   sSql = sSql & " ORDER BY MRP_ACTIONDATE"
   'End If
   ...
   'bSqlRows = clsADOCon.GetQuerySet(sSql, RdoMrpEx)
   If bSqlRows Then
      With RdoMrpEx
         Do Until .EOF
            ...
            If (bRet = True) Then
               ...
               If (optDtPart = 1) Then
                  ' Find any part shortage
                  Dim RdoMrpParts As ADODB.Recordset
                  sSql = "SELECT BMPARTREF, BMQTYREQD, PAQOH, PAQRUNTOT FROM tempMrplPartShort" & vbCrLf _
                           & " WHERE BMASSYPART = '" & strAssyPart & "' and RUNNO = " & strRunNo & " AND Convert(varchar(10), MRP_ACTIONDATE, 101) ='" & strPartDtRqd & "'" & vbCrLf _
                           & strExp & " ORDER BY BMPARTREF, MRP_ACTIONDATE"
                  
                  Debug.Print sSql
                  
                  bSqlRows = clsADOCon.GetDataSet(sSql, RdoMrpParts, ES_KEYSET)
                  If bSqlRows Then
                     With RdoMrpParts
                        Do Until .EOF
                           ...
                           Grd.Col = 1
                           Grd.Text = "  " & Trim(!BMPARTREF)
                           Grd.Col = 3
                           Grd.Text = Trim(!BMQTYREQD)
                           Grd.Col = 4
                           Grd.Text = Trim(!PAQOH)
                           Grd.Col = 5
                           Grd.Text = Trim(!PAQRUNTOT)
                  ...
                  Set RdoMrpParts = Nothing
               End If
               
            End If ' if it is not 0120
            .MoveNext
         Loop
      End With
   End If
...
Private Sub GrdAddHeader()
     ...
     With Grd
      ...
      .Text = "Qty (ReqQty)"
      .Col = 4
      .Text = "Required Date (PAQOH)"
      .Col = 5
      .Text = "Action Date (RunTot)"
      .Col = 6
      .Text = "PL Stat"
      .Col = 7
      .Text = "SC Stat"
...
Private Sub SaveOptions()
   ...
   sClass = cmbCls
   sBuyer = cmbByr
   SaveSetting "Esi2000", "EsiProd", "MrplMRe04", sOptions
