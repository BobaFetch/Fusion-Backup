...
Function ASNOutFile(nFileNum As Integer, strDate As String) As Integer
   ...
   bSqlRows = clsAdoCon.GetDataSet(sSql, rdoPS, rdOpenStatic)
...
MsgBox "@@@ CreateInvoiceEDIFile " & strFullpath
   ...
   If (Trim(strFileName) <> "") Then
      ' Open the file
      nFileNum = FreeFile
MsgBox "@@@ Open file" & strFullpath & " for file number " & CStr(nFileNum)
      Open strFullpath For Output As nFileNum
      
MsgBox "@@@ check for EOF"
      If EOF(nFileNum) Then
MsgBox "@@@ GenerateINVFile"
...
Function GenerateINVFile(ByVal nFileNum As Integer, ByVal strDate As String) As Integer
   ...
   sSql = "SELECT PIPART, PARTNUM, INVNO, PSNUMBER, INVTOTAL, INVDATE, INVCUST, SOPO, " _
    ...
    If bSqlRows Then
MsgBox "@@@ GenerateINVFile sql returns results"
        With rdoPS
            While Not .EOF
...
Function AddEDIFieldsLength(ByVal strType As String, ImpType As String)
   ...
   If (strType <> "") Then
      ...
      'ReDim arrValue(0 To adodb.recordcount + 1)
      ReDim arrValue(0 To RdoEdi.RecordCount + 1)
      'ReDim arrFieldName(0 To adodb.recordcount + 1)
