...
Public Const MAX_CONCURRENT_LOGINS = 100
...
'   gblnSqlRows = GetDataSet(rdoJrn, ES_FORWARD)
...
Public Function GetEmployee(pintPIN As Long, ByRef pempGetEmployee As Employee) As Boolean
   ...
   End If
   sSql = "SELECT PREMLSTNAME,PREMFSTNAME,PREMPAYRATE," _
...
Public Function GetSystemMessage() As String
   ...
   If (Not pRdoMsg Is Nothing) Then
   ...
   If gblnSqlRows Then
      With prdoJobs
         While Not .EOF
            ...
            'pjobMyJobs(jobs).strPart = .rdoColumns(0)
            .MoveNext
            jobs = jobs + 1
         Wend
      End With
...
Public Sub OpenIndirectTC(pempEmployee As Employee)
   ...
   If Not clsADOCon.GetDataSet(sSql, prdoExists) Then
...
Public Sub CloseIndirectTC(pempEmployee As Employee)
   ...
   Dim tc As New ClassTimeCharge
   Dim tcid As String
   tcid = tc.GetTimeCardID(pempEmployee.intNumber, startTime)
   ...
   tc.UpdateTimeCardTotals pempEmployee.intNumber, startTime
...
Public Sub PunchIn(pempEmployee As Employee)
   ...
   'if there are direct charge logins, there should be no indirect charges
   Dim rdo As ADODB.Recordset
   sSql = "select * from IstcTable where ISINDIRECT = 0 and ISEMPLOYEE = " & pempEmployee.intNumber
   If clsADOCon.GetDataSet(sSql, rdo) Then
      'MsgBox "Deleting overlapping indirect charge started " & Format(rdo!ISMOSTART, "mm/dd/yy hh:mm AM/PM")
   ...
   'MM RdoCon.Execute sSql, rdExecDirect
   clsADOCon.ExecuteSQL sSql
...
Public Sub LogOffJob( _
                     ...
                     IsUserLoggingOff As Boolean, Optional Comments As String)
   ...
   sSql = "SELECT ISMOSTART, ISSHOP, ISWCNT FROM IstcTable " & vbCrLf _
   ...
   If gblnTime And LoginTime <> LogoutTime Then
      Dim tc As New ClassTimeCharge
      TimeCardID = tc.GetTimeCardID(pempEmployee.intNumber, LoginTime)   'get timechard for login date
      ...
      If tc.CreateTimeCharge(TimeCardID, pempEmployee.intNumber, LoginTime, LogoutTime, _
         ...
         psngAccept, psngReject, psngScrap, Comments) Then
         
            tc.ComputeOverlappingCharges CLng(pempEmployee.intNumber), LoginTime
            
            tc.UpdateTimeCardTotals pempEmployee.intNumber, LoginTime
            clsADOCon.CommitTrans
...
'    sSql = "SELECT GETDATE()"
'    gblnSqlRows = GetDataSet(prdoTme)
'    If gblnSqlRows Then
'        With prdoTme
'            GetServerDateTime = .rdoColumns(0)
'        End With
'    End If
'    Set prdoTme = Nothing
'End Function
...
Public Sub LogInToJob(sPartNo As String, nRunNo As Integer, nOpNo As Integer)
   ...
   sSql = "INSERT INTO IstcTable " & vbCrLf _
          & "(ISEMPLOYEE,ISMO,ISRUN,ISOP,ISMOSTART,ISSHOP,ISWCNT) " & vbCrLf _
