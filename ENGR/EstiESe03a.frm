...
Begin VB.Form EstiESe03a 
   AutoRedraw      =   -1  'True
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Complete Estimates"
   ClientHeight    =   4845
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   7185
...
Private Sub FillGrid()
   ...
   sSql = "SELECT BIDREF,BIDNUM,BIDPRE,BIDPART,BIDCUST,BIDQUANTITY," _
          ...
          & "AND BIDPART=PARTREF AND BIDCANCELED=0 AND BIDCOMPLETE=0 " _
          & "AND BIDREF>=" & Val(txtBid) & " AND BIDCUST LIKE '" _
...
Private Sub UpdateBids()
   ...
   Next
   If b = 0 Then
      MsgBox "No Estimates Were Selected For Completion.", _
         vbInformation, Caption
   Else
      vDate = Format(txtAcp, "mm/dd/yyyy")
      sMsg = "This Procedure Marks All Selected Estimates " & vbCrLf _
             & "Completed Would Like To Continue   " & vbCrLf _
      ...
      If b = vbYes Then
         ...
         For iList = 1 To iTotalRows
            If sBids(iList, 1) = "X" Then b = 1 _
                     Else b = 0
            sSql = "UPDATE EstiTable SET BIDCOMPLETED='" & vDate & "'," _
                   & "BIDCOMPLETE=" & b & " WHERE BIDREF=" & Val(sBids(iList, 0)) & " "
            clsADOCon.ExecuteSQL sSql 'rdExecDirect
         Next
         If clsADOCon.ADOErrNum = 0 Then
            MsgBox "Marked Estimates Completed.", _
               vbInformation, Caption
            FillGrid
         Else
            MsgBox "Marked Estimates Were Not Completed.", _
