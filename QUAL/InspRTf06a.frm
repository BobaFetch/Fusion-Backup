...
Begin VB.Form InspRTf06a 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Delete A Responsibility Code"
   ClientHeight    =   2385
   ClientLeft      =   2385
   ClientTop       =   1680
   ClientWidth     =   5865
   ...
   Begin VB.CommandButton cmdDel 
      ...
      Left            =   4920
      TabIndex        =   5
      ToolTipText     =   "Delete This Responsibility Code"
      Top             =   600
      Width           =   875
   End
   ...
   Begin VB.ComboBox cmbCde 
      ...
      Sorted          =   -1  'True
      TabIndex        =   0
      ToolTipText     =   "Select Responsibility Code From List"
      Top             =   810
      Width           =   1675
   End
...
Private Sub cmdDel_Click()
   If bGoodCode = 0 Then
      MsgBox "Requires A Valid Responsibility Code.", _
...
Private Function GetCode() As Byte
   ...
   MouseCursor 13
   On Error GoTo DiaErr1
   sSql = "SELECT RESREF,RESNUM,RESDESC,RESNOTES " _
          & "FROM RjrsTable WHERE RESREF='" & sRejCode & "'"
   bSqlRows = clsADOCon.GetDataSet(sSql, RdoCde, ES_FORWARD)
   If bSqlRows Then
      With RdoCde
         cmbCde = "" & Trim(!RESNUM)
         txtDsc = "" & Trim(!RESDESC)
         txtCmt = "" & Trim(!RESNOTES)
      End With
      GetCode = 1
   Else
      GetCode = 0
...
Private Sub DeleteCode()
   ...
   sMsg = "You Are About To Pemanently Remove This" & vbCr _
          & "Responsibility Code. Do You Wish To Continue?"
   bResponse = MsgBox(sMsg, ES_NOQUESTION, Caption)
   If bResponse = vbYes Then
      sSql = "SELECT DISTINCT RITRESPCODE FROM RjitTable " _
             & "WHERE RITRESPCODE='" & Compress(cmbCde) & "'"
      ...
      If clsADOCon.RowsAffected = 0 Then
         ...
         If bResponse = vbYes Then
            ...
            sSql = "DELETE FROM RjrsTable WHERE " _
                   & "RESREF='" & Compress(cmbCde) & "'"
            clsADOCon.ExecuteSQL sSql
      ...
      Else
         MsgBox cmbCde & " Is Used On One Or More Inspection Reports." & vbCr _
            & "Cannot Delete This Responsibility Code.", _
            vbExclamation, Caption
      End If
   Else
      CancelTrans
