...
Begin VB.Form InspRTf08a 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Delete A Cause Code"
   ClientHeight    =   2385
   ClientLeft      =   2385
   ClientTop       =   1680
   ClientWidth     =   5865
   ...
   Begin VB.CommandButton cmdDel 
      ...
      Left            =   4920
      TabIndex        =   5
      ToolTipText     =   "Delete This Cause Code"
      Top             =   600
      Width           =   875
   End
...
Private Function GetCode() As Byte
   ...
   MouseCursor 13
   On Error GoTo DiaErr1
   sSql = "SELECT CAUSEREF,CAUSENUM,CAUSEDESC,CAUSENOTES " _
          & "FROM RjcaTable WHERE CAUSEREF='" & sRejCode & "'"
   bSqlRows = clsADOCon.GetDataSet(sSql, RdoCde, ES_FORWARD)
   If bSqlRows Then
      With RdoCde
         cmbCde = "" & Trim(!CAUSENUM)
         txtDsc = "" & Trim(!CAUSEDESC)
         txtCmt = "" & Trim(!CAUSENOTES)
      End With
      GetCode = 1
   Else
      GetCode = 0
...
Private Sub DeleteCode()
   ...
   sMsg = "You Are About To Pemanently Remove This" & vbCr _
          & "Cause Code. Do You Wish To Continue?"
   bResponse = MsgBox(sMsg, ES_NOQUESTION, Caption)
   If bResponse = vbYes Then
      sSql = "SELECT DISTINCT RITCAUSE FROM RjitTable " _
             & "WHERE RITCAUSE='" & Compress(cmbCde) & "'"
      ...
      If clsADOCon.RowsAffected = 0 Then
         ...
         If bResponse = vbYes Then
            ...
            sSql = "DELETE FROM RjcaTable WHERE " _
                   & "CAUSEREF='" & Compress(cmbCde) & "'"
            clsADOCon.ExecuteSQL sSql
      ...
      Else
         MsgBox cmbCde & " Is Used On One Or More Inspection Reports." & vbCr _
            & "Cannot Delete This Cause Code.", _
            vbExclamation, Caption
      End If
   Else
      CancelTrans
