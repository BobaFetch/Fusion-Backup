...
Begin VB.Form FainFAf02a 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Cancel A First Article Report"
   ClientHeight    =   2355
   ClientLeft      =   2685
   ClientTop       =   1425
   ClientWidth     =   6030
   ...
   Begin VB.CommandButton cmdOk 
      ...
      TabIndex        =   4
      TabStop         =   0   'False
      ToolTipText     =   "Cancel The First Article Report"
      Top             =   520
      Width           =   875
   End
   ...
   Begin VB.Label z1 
      ...
      Caption         =   "Deletes The Selected Report"
      Height          =   288
      Index           =   2
      Left            =   240
      TabIndex        =   8
...
Private Sub cmdOk_Click(Index As Integer)
   If Len(Trim(cmbPrt)) > 4 Then
      ...
      If bGoodReport Then DeleteReport
   End If
...
Private Sub DeleteReport()
   ...
   sMsg = "This Function Permanently Removes All Records" & vbCr _
          & "Of The Selected Report. Do You Wish To Continue?"
   ...
   If bResponse = vbYes Then
      ...
      sSql = "DELETE FROM FaitTable WHERE FA_ITNUMBER='" _
             & Compress(cmbPrt) & "' AND FA_ITREVISION='" _
             & cmbRev & "'"
      clsADOCon.ExecuteSQL sSql
      
      sSql = "DELETE FROM FadcTable WHERE FA_DOCNUMBER='" _
             & Compress(cmbPrt) & "' AND FA_DOCREVISION='" _
             & cmbRev & "'"
      clsADOCon.ExecuteSQL sSql
      
      sSql = "DELETE FROM FahdTable WHERE FA_REF='" _
             & Compress(cmbPrt) & "' AND FA_REVISION='" _
             & cmbRev & "'"
      clsADOCon.ExecuteSQL sSql
      
'      If Err = 0 Then
'         sMsg = "This Report Is About To Be Permanently Removed" & vbCr _
'                & "Are You Sure That You Wish To Continue?'
'         bResponse = MsgBox(sMsg, ES_NOQUESTION, Caption)
'         If bResponse = vbYes Then
            clsADOCon.CommitTrans
            MsgBox "The Report Was Canceled.", _
               vbInformation, Caption
            FillCombo
'         Else
...
'         End If
'      Else
'         MsgBox "Couldn't Delete The Report.", _
