...
Begin VB.Form InspRTp05a 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Inspection Reports By Customer"
   ClientHeight    =   3420
   ClientLeft      =   2115
   ClientTop       =   1125
   ClientWidth     =   7260
...
Private Sub PrintReport()
   ...
   End If
   If Trim(cmbCst) <> "ALL" Then sCode = Compress(cmbCst)
   
   On Error GoTo DiaErr1
   sCustomReport = GetCustomReport("quarj05")
   ...
   sSql = "{RjhdTable.REJCUST} LIKE '" & sCode & "*' AND " _
...
Private Sub SaveOptions()
   ...
   SaveSetting "Esi2000", "EsiQual", "rj05", Trim(sOptions)
...
Private Sub GetOptions()
   ...
   Dim sCom As String
   On Error Resume Next
   sOptions = GetSetting("Esi2000", "EsiQual", "rj05", Trim(sOptions))
   If Len(Trim(sOptions)) > 0 Then
      cmbCst = Trim(Mid(sOptions, 1, 10))
