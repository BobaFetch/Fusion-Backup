...
Begin VB.Form FainFAe02b 
   ...
   Begin VB.CommandButton cmdPrn 
      DownPicture     =   "FainFAe02b.frx":07AE
      Height          =   320
      Left            =   5880
      Picture         =   "FainFAe02b.frx":0938
      Style           =   1  'Graphical
      TabIndex        =   19
      TabStop         =   0   'False
...
Private Sub Form_Load()
   ...
   'tabFrame(1).Left = 180
   tabFrame(1).Left = (tab1.Width - tabFrame(1).Width) / 2
'
   FormatControls
   bOnLoad = 1
...
Private Sub txtDno_LostFocus(Index As Integer)
   txtDno(Index) = CheckLen(Trim(txtDno(Index)), 30)
...
Private Sub txtDsh_LostFocus(Index As Integer)
   txtDsh(Index) = CheckLen(Trim(txtDsh(Index)), 6)
