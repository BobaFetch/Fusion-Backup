...
Private Sub cmdHlp_Click()
   If cmdHlp Then
      MouseCursor 13
      'OpenHelpContext 0
      MsgBox "No Help Context Has Been Issued For This Item.", _
...
Private Sub AddCode()
   ...
   On Error GoTo DiaErr1
   sSql = "Select * FROM RjcaTable"
   bSqlRows = clsADOCon.GetDataSet(sSql, RdoCde, ES_KEYSET)
   RdoCde.AddNew
   RdoCde!CAUSEREF = "" & sNewCode
   RdoCde!CAUSENUM = "" & cmbCde
   RdoCde.Update
   MouseCursor 0
...
Private Function GetCode() As Byte
   ...
   MouseCursor 13
   On Error GoTo DiaErr1
   sSql = "SELECT *  FROM RjcaTable WHERE CAUSEREF='" & sRejCode & "'"
   bSqlRows = clsADOCon.GetDataSet(sSql, RdoCde, ES_KEYSET)
   If bSqlRows Then
      With RdoCde
