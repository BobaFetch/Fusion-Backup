...
Begin VB.Form DockODe02a 
   ...
   Begin VB.CommandButton cmdCmt 
      DownPicture     =   "DockODe02a.frx":0000
      Height          =   320
      Index           =   1
      Left            =   960
      Picture         =   "DockODe02a.frx":04DA
      Style           =   1  'Graphical
      TabIndex        =   50
      TabStop         =   0   'False
   ...
   End
   Begin VB.CommandButton cmdCmt 
      DownPicture     =   "DockODe02a.frx":0ADC
      Height          =   320
      Index           =   2
      Left            =   960
      Picture         =   "DockODe02a.frx":0FB6
      Style           =   1  'Graphical
      TabIndex        =   49
      TabStop         =   0   'False
      ToolTipText     =   "PO Item Comments"
   ...
   End
   Begin VB.CommandButton cmdCmt 
      DownPicture     =   "DockODe02a.frx":15B8
      Height          =   320
      Index           =   3
      Left            =   960
      Picture         =   "DockODe02a.frx":1A92
      Style           =   1  'Graphical
      TabIndex        =   48
      TabStop         =   0   'False
      ToolTipText     =   "PO Item Comments"
   ...
   End
   Begin VB.CommandButton cmdCmt 
      DownPicture     =   "DockODe02a.frx":2094
      Height          =   320
      Index           =   4
      Left            =   960
      Picture         =   "DockODe02a.frx":256E
      Style           =   1  'Graphical
      TabIndex        =   47
      TabStop         =   0   'False
      ToolTipText     =   "PO Item Comments"
   ...
   Begin VB.CommandButton cmdUp 
      DisabledPicture =   "DockODe02a.frx":2B70
      DownPicture     =   "DockODe02a.frx":3062
      Height          =   372
      Left            =   6400
      MaskColor       =   &H00000000&
      Picture         =   "DockODe02a.frx":3554
      Style           =   1  'Graphical
      TabIndex        =   46
      TabStop         =   0   'False
      Top             =   4920
   ...
   Begin VB.CommandButton cmdDn 
      DisabledPicture =   "DockODe02a.frx":3A46
      DownPicture     =   "DockODe02a.frx":3F38
      Height          =   372
      Left            =   6840
      MaskColor       =   &H00000000&
      Picture         =   "DockODe02a.frx":442A
      Style           =   1  'Graphical
      TabIndex        =   45
      TabStop         =   0   'False
      Top             =   4920
...
Private Const POITEM_Comment = 11
...
Private Function GetPOShortQtyFlag()
   
   Dim RdoPOqty As ADODB.Recordset
   sSql = "SELECT ISNULL(COALLOWDELSHORTQTY, 0) COALLOWDELSHORTQTY FROM ComnTable WHERE COREF=1"
   
   bSqlRows = clsADOCon.GetDataSet(sSql, RdoPOqty, ES_FORWARD)
   If bSqlRows Then
      With RdoPOqty
         GetPOShortQtyFlag = IIf(!COALLOWDELSHORTQTY = 0, False, True)
         ClearResultSet RdoPOqty
      End With
   Else
      GetPOShortQtyFlag = False
   End If
...
Private Sub GetPoItems()
   ...
   sSql = "SELECT PINUMBER,PIITEM,PIREV,PITYPE,PIPART,PIPQTY," _
      & "PIODDELPSNUMBER,PIODDELQTY,PIODDELDATE,PICOMT,PARTREF,PARTNUM,PADESC" & vbCrLf _
   ...
   If bSqlRows Then
      With RdoGpi
         Do Until .EOF
            ...
            vItems(iRow, POITEM_Comment) = "" & Trim(!PICOMT)
            If Not IsNull(!PIODDELDATE) Then
               vItems(iRow, POITEM_DeliveredDate) = "'" & Format(!PIODDELDATE, "mm/dd/yyyy") & "'"
            Else
               vItems(iRow, POITEM_DeliveredDate) = Format(Now, "mm/dd/yyyy")
            End If
            If iRow < 5 Then
...
Private Sub UpdateList()
   ...
   ' get the flag to allow PO short Qty
   bShortQty = False
   bShortQty = GetPOShortQtyFlag
   ...
   For iRow = 1 To iTotalItems
      ...
      If recQty > 0 Then
         If recQty < ordQty Then
            ...
            bCompPOit = False
            If (bShortQty = True) Then
               sMsg = "Do you want to commplete receiving the PO line item?"
               If MsgBox(sMsg, ES_YESQUESTION, Caption) = vbYes Then
                  bCompPOit = True
               End If
            End If
...
'            & "and PIREV='" & PoRev & "'"
         sSql = "update PoitTable" & vbCrLf _
            ...
            & "PIODDELDATE='" & Trim(txtDte) & "'," & vbCrLf _
            & "PIODDELQTY=" & recQty & "," & vbCrLf _
            & "PIODDELPSNUMBER='" & vItems(iRow, POITEM_PSNumber) & "'," & vbCrLf _
            & "PIPQTY=" & recQty & vbCrLf _
            & "where PINUMBER=" & poNumber & vbCrLf _
         ...
         'vItems(iRow, POITEM_DeliveredDate)
         
         'if this is a partial delivery, create a split PO item for the difference
                 ' if the option to complete the PO is selected then create a new PO and Cancel the items short
         If recQty < ordQty Then
            Dim item As New ClassPoItem
            If (bCompPOit = True) Then
               item.CancelPOItem poNumber, PoRelease, poItem, poRev, ordQty - recQty
            Else
...
Private Sub showItem(indexItem As Integer, showThisItem As Boolean)
      ...
      txtAcc(indexItem).Visible = showThisItem
