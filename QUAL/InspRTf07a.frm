...
Begin VB.Form InspRTf07a 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Delete A Disposition Code"
   ClientHeight    =   2385
   ClientLeft      =   2385
   ClientTop       =   1680
   ClientWidth     =   5865
   ...
   Begin VB.CommandButton cmdDel 
      ...
      Left            =   4920
      TabIndex        =   5
      ToolTipText     =   "Delete This Disposition Code"
      Top             =   600
      Width           =   875
   End
...
Private Sub cmdDel_Click()
   If bGoodCode = 0 Then
      MsgBox "Requires A Valid Disposition Code.", _
...
Private Function GetCode() As Byte
   ...
   MouseCursor 13
   On Error GoTo DiaErr1
   sSql = "SELECT DISREF,DISNUM,DISDESC,DISNOTES " _
          & "FROM RjdsTable WHERE DISREF='" & sRejCode & "'"
   bSqlRows = clsADOCon.GetDataSet(sSql, RdoCde, ES_FORWARD)
   If bSqlRows Then
      With RdoCde
         cmbCde = "" & Trim(!DISNUM)
         txtDsc = "" & Trim(!DISDESC)
         txtCmt = "" & Trim(!DISNOTES)
      End With
      GetCode = 1
   Else
      GetCode = 0
...
Private Sub DeleteCode()
   ...
   sMsg = "You Are About To Pemanently Remove This" & vbCr _
          & "Disposition Code. Do You Wish To Continue?"
   bResponse = MsgBox(sMsg, ES_NOQUESTION, Caption)
   If bResponse = vbYes Then
      sSql = "SELECT DISTINCT RITDISPCODE FROM RjitTable " _
             & "WHERE RITDISPCODE='" & Compress(cmbCde) & "'"
      ...
      If clsADOCon.RowsAffected = 0 Then
         ...
         If bResponse = vbYes Then
            ...
            sSql = "DELETE FROM RjdsTable WHERE " _
                   & "DISREF='" & Compress(cmbCde) & "'"
            clsADOCon.ExecuteSQL sSql
      ...
      Else
         MsgBox cmbCde & " Is Used On One Or More Inspection Reports." & vbCr _
            & "Cannot Delete This Disposition Code.", _
            vbExclamation, Caption
      End If
   Else
      CancelTrans
