...
'   strFilePath = strFilePath & "Forcast"
   ...
   bSqlRows = GetDataSet(RdoHdEdi, ES_KEYSET)
...
Public Sub GetNewSO(ByRef sNewSo As String, ByVal sSoType As String)
   ...
   bSqlRows = GetDataSet(RdoSon, ES_FORWARD)
   If bSqlRows Then
      With RdoSon
...
Dim strFileName As String
   ...
   'ImportEDISO strFilePath, strFileName
...
Function Fill850Grid(strCust As String) As Integer
   ...
   bSqlRows = GetDataSet(RdoHdEdi, rdOpenStatic)
...
Private Function CheckOfExistingSO(strPONumber As String, strPartID As String, ByRef strSoNum As String) As Boolean
   ...
   bSqlRows = GetDataSet(RdoSO, ES_FORWARD)
   If bSqlRows Then
      With RdoSO
...
End Function
 ...
 Private Function GetPartPrice(strPart As String, ByRef strPrice As String)
   ...
   bSqlRows = GetDataSet(RdoPrice, ES_FORWARD)
   If bSqlRows Then
      With RdoPrice
...
Private Sub FillEDICust()
   ...
   If bSqlRows Then
      With RdoCst
         Do Until .EOF
            AddComboStr cmbCst.hwnd, "" & Trim(.rdoColumns(0))
            .MoveNext
         Loop
         ClearResultSet RdoCst
      End With
...
Public Sub GetLastSalesOrder(sOldSo As String, sNewSo As String, bFillText As Boolean)
   ...
   bSqlRows = GetDataSet(RdoSon, ES_FORWARD)
   If bSqlRows Then
      If Val(Right$(RdoSon!COLASTSALESORDER, 5)) > 0 Then _
             lblLst = RdoSon!COLASTSALESORDER
   End If
   If Trim(lblLst) = "" Then
      ...
      If bSqlRows Then
         With RdoSon
            If Not IsNull(.rdoColumns(0)) Then
               lblLst = "" & Trim(!SOTYPE) & Format$(!SalesOrder, "00000")
            Else
               lblLst = "S00000"
            End If
            ClearResultSet RdoSon
      ...
      End If
   End If
   sNewSo = Format(Val(Right(lblLst, 5)) + 1, "00000")
   If bFillText Then txtSon = sNewSo
   sOldSo = lblLst
   Set RdoSon = Nothing
   Exit Sub
