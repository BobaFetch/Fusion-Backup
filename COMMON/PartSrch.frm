...
Begin VB.Form ViewParts 
   ...
   Begin VB.CheckBox cbHideObsolete 
      BackColor       =   &H80000018&
      ...
      Width           =   2535
   End
   Begin VB.CheckBox cbHideInactive 
      BackColor       =   &H80000018&
...
'   If txtPrt <> "ALL" Then sSearchStr = UCase$(Compress(txtPrt))
   If optDsc.value = vbUnchecked Then
      ...
      If cbHideInactive = vbChecked Then sSql = sSql & " AND PAINACTIVE = 0 "
      If cbHideObsolete = vbChecked Then sSql = sSql & " AND PAOBSOLETE = 0 "
      sSql = sSql & AdditionalWhereClause
...
Private Function AdditionalWhereClause() As String
    AdditionalWhereClause = ""
    If Len(Trim(lblWhereClause)) = 0 Then Exit Function
    AdditionalWhereClause = " AND (" & Replace(lblWhereClause, "WHERE", "") & ")"
End Function


Private Sub GetSettings()
    Dim AdoPref As ADODB.Recordset
    
    sSql = "SELECT ISNULL(0,COHIDEINACTIVEPART), ISNULL(0,COHIDEOBSOLETEPART) FROM ComnTable WHERE COREF=1 "
    bSqlRows = clsADOCon.GetDataSet(sSql, AdoPref, ES_FORWARD)
    If bSqlRows Then
        cbHideObsolete.value = AdoPref!COHIDEOBSOLETEPART
        cbHideInactive.value = AdoPref!COHIDEINACTIVEPART
    Else
        cbHideObsolete.value = vbUnchecked
        cbHideInactive.value = vbUnchecked
    End If
    Set AdoPref = Nothing
