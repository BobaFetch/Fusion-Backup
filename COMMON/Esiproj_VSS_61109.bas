...
Public Function GetCurrentPart(PartNumber As String, ByRef PartDescription As Control, _
   ...
   Dim RdoCur As rdoResultset
   On Error GoTo ModErr1
   sSql = "Qry_GetINVCfindPart '" & Compress(PartNumber) & "'"
   bSqlRows = GetDataSet(RdoCur, ES_FORWARD)
   If bSqlRows Then
      With RdoCur
...
Public Sub FillCustomers(Optional frm As Form, Optional ShowAll As Boolean)
   ...
   If bSqlRows Then
      With RdoCst
         Do Until .EOF
            AddComboStr combo.hwnd, "" & Trim(.rdoColumns(0))
            .MoveNext
         Loop
...
'   'Calling Debug is ignored except in VB, so it will produce an error if running in VB
'
'   RunningInIDE = 0
'   'Exit Function   'test security when not in IDE
'
   ...
   For iList = 0 To frm.Controls.Count - 1
      ...
      ElseIf TypeOf frm.Controls(iList) Is CommandButton Then
         ...
         If frm.Controls(iList).Name = "cmdHlp" Then
            frm.Controls(iList).PictureUp = MdiSect.XPHelpUp.Picture
            frm.Controls(iList).PictureDn = MdiSect.XPHelpDn.Picture
...
Public Sub FillRegions(Optional frm As Form)
   ...
   bSqlRows = GetDataSet(RdoReg, ES_FORWARD)
   If bSqlRows Then
      With RdoReg
         Do Until .EOF
            If frm Is Nothing Then
...
Public Sub FillStatusCode(ByRef cmbStatCode As ComboBox, Optional frm As Form)
   Dim RdoStatCd As rdoResultset
   On Error GoTo ModErr1
   sSql = "Qry_FillStatCode"
   bSqlRows = GetDataSet(RdoStatCd, ES_FORWARD)
   If bSqlRows Then
      With RdoStatCd
         Do Until .EOF
            If frm Is Nothing Then
            ...
            .MoveNext
         Loop
         ClearResultSet RdoStatCd
      End With
   End If
   Set RdoStatCd = Nothing
   Exit Sub
...
Public Sub FillProductCodes(Optional frm As Form)
   ...
   bSqlRows = GetDataSet(RdoCde, ES_FORWARD)
   If bSqlRows Then
      With RdoCde
         Do Until .EOF
...
Public Sub FillTerms(Optional frm As Form)
   ...
   bSqlRows = GetDataSet(RdoTrm, ES_FORWARD)
   If bSqlRows Then
      With RdoTrm
         Do Until .EOF
            If frm Is Nothing Then
...
Public Sub ActivateSection(sCurrentSection As String)
   ...
   MdiSect.Logo(0).ToolTipText = "ESI Software Engineering Inc"
   MdiSect.Logo(1).ToolTipText = "ESI Software Engineering Inc"
...
'Standard procedure for receiving Select Query resultsets
'bSqlRows = GetDataSet (RdoResults, ES_FORWARD)
'See also GetQuerySet for VB built queries

'Use local error Trapping - many errors here may not require logging
'requires "on error goto whoops"
'...
'whoops: ...
'  DoModuleErrors frm

Public Function GetDataSet(RdoDataSet As rdoResultset, Optional iCursorType As Integer) As Boolean
   Err.Clear
   GetDataSet = False
   bUserAction = True   'don't timeout SQL connection
...
'bSqlRows = GetQuerySet (RdoResults, RdoQry, ES_FORWARD)
'See also GetDataSet for general Select queries

Public Function GetQuerySet(RdoDataSet As rdoResultset, RdoQueryDef As rdoQuery, Optional iCursorType As Integer) As Boolean
   ...
   Else
      ...
      Else
         ...
         End If
      End If
      If Err > 0 Then GetQuerySet = False
   End If
   'Clear it
   sSql = ""
...
Public Function CheckLotTracking() As Byte
   ...
   If bSqlRows Then ClearResultSet RdoLots
   CheckLotTracking = 1
   Set RdoLots = Nothing
   Exit Function
...
Public Function GetRemainingLotQty(LotPart As String, _
   ...
   bSqlRows = GetDataSet(RdoQty, ES_FORWARD)
   If bSqlRows Then
      With RdoQty
         Do Until .EOF
...
Public Function GetLotRemainingQty(LotPart As String) As Currency
   ...
   If GetDataSet(RdoQty, ES_FORWARD) Then
      GetLotRemainingQty = RdoQty.rdoColumns(0)
   End If
...
Public Function GetTimeFormat() As String
   Dim rdoFmt As rdoResultset
   
   On Error GoTo ModErr1
   sSql = "SELECT TimeFormat FROM Preferences WHERE PreRecord=1"
   bSqlRows = GetDataSet(rdoFmt, ES_FORWARD)
   If bSqlRows Then
      With rdoFmt
         If Not IsNull(.rdoColumns(0)) Then
            GetTimeFormat = "" & Trim(.rdoColumns(0))
         Else
            GetTimeFormat = ES_QuantityDataFormat
         End If
         ClearResultSet rdoFmt
      End With
   Else
      GetTimeFormat = ES_QuantityDataFormat
   End If
...
Public Sub LoadComboBoxAndSelect(cbo As ComboBox, Optional SelectString As String)
   ...
   End If
   If sSql = "" Then Exit Sub
   Set rdo = RdoCon.OpenResultset(sSql, rdOpenForwardOnly, rdConcurReadOnly)
   ...
   If Not ComboLoad.BOF And Not ComboLoad.EOF Then
      With ComboLoad
         Do Until .EOF
            ...
            AddComboStr Cntrl.hwnd, Format(.rdoColumns(ColumnNumber), ColumnFormat)
            .MoveNext
         Loop
         ClearResultSet ComboLoad
      End With
...
Public Function GetPartAccounts(PartNumber As String, _
   ...
   If GetDataSet(rdoAct, ES_FORWARD) Then
      DebitAccount = "" & rdoAct.rdoColumns(0)
   End If
   
   sSql = "select dbo.fnGetPartInvAccount ( '" & compressedPart & "' )"
   If GetDataSet(rdoAct, ES_FORWARD) Then
      CreditAccount = "" & rdoAct.rdoColumns(0)
   End If
...
Public Sub UpdateWipColumns(InventoryRows As Long)
   ...
   Dim sWipClab As String
   Dim sWipCMat As String
   Dim sWipCExp As String
   Dim sWipCOhd As String
   Dim sWipDlab As String
   Dim sWipDMat As String
   Dim sWipDExp As String
   Dim sWipDOhd As String
   ...
   If bSqlRows Then
      With RdoInv
         sWipClab = "" & Trim(!WIPCRLABACCT)
         sWipCMat = "" & Trim(!WIPCRMATACCT)
         sWipCExp = "" & Trim(!WIPCREXPACCT)
         sWipCOhd = "" & Trim(!WIPCROHDACCT)
         
         sWipDlab = "" & Trim(!WIPDRLABACCT)
         sWipDMat = "" & Trim(!WIPDRMATACCT)
         sWipDExp = "" & Trim(!WIPDREXPACCT)
         sWipDOhd = "" & Trim(!WIPDROHDACCT)
         .Cancel
      End With
      ClearResultSet RdoInv
   End If
   sSql = "UPDATE InvaTable SET " _
          & "INCRLABACCT='" & sWipClab & "'," _
          & "INCRMATACCT='" & sWipCMat & "'," _
          & "INCREXPACCT='" & sWipCExp & "'," _
          & "INCROHDACCT='" & sWipCOhd & "'," _
          & "INDRLABACCT='" & sWipClab & "'," _
          & "INDRMATACCT='" & sWipCMat & "'," _
          & "INDREXPACCT='" & sWipCExp & "'," _
          & "INDROHDACCT='" & sWipCOhd & "' " _
...
lngImgLen = RdoPic.rdoColumns(1).ColumnSize
intBlocks = lngImgLen \ lngBlockSize
intBlocksLo = lngImgLen Mod lngBlockSize

ReDim bytData(intBlocksLo)
bytData() = RdoPic.rdoColumns(1).GetChunk(intBlocksLo)
Put DataFile, , bytData()

For intCnt = 1 To intBlocks
    bytData() = RdoPic.rdoColumns(1).GetChunk(lngBlockSize)
    Put DataFile, , bytData()
Next intCnt
