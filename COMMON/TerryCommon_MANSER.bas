...
Public Sub LoadComboWithOpenOpsForRun_Old(cbo As ComboBox, sPart As String, nRun As Long)
...
Public Sub LoadComboWithOpenOpsForRun(cbo As ComboBox, sPart As String, nRun As Long)
   ...
   bTCSerOp = GetTCServiceOp()
    ...
    Else
         ' MM 6/1/2010
        If (bOnlyOpenOp = True) Then
            sSql = "SELECT OPNO, rtrim(WCNDESC) as WCNDESCFROM RnopTable ro " & vbCrLf _
        ...
        Else
            ...
            sSql = "SELECT OPNO, rtrim(WCNDESC) as WCNDESC FROM RnopTable ro " & vbCrLf _
                     & "join WcntTable wc on wc.WCNREF = ro.OPCENTER" & vbCrLf _
                     & "WHERE OPREF = '" & Compress(sPart) & "'" & vbCrLf _
                     & "AND OPRUN = " & nRun & vbCrLf _
                     & "AND (LTRIM(RTRIM(OPSERVPART)) = '' OR OPSERVPART IS NULL) " & vbCrLf _
