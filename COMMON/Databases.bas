...
' BBS 7/21/2010 Changed Database version from 70 to 71
' BBS 9/7/2010 Changed database version from 72 to 73
' BBS 9/21/2010 Changed database version from 73 to 74
' MM 11/22/2010 Changed database version from 76 to 77
' BBS 11/23/2010 Changed database version from 77 to 78 for Ticket #41047
' BBS 12/07/2010 Changed database version from 78 to 79 for Ticket #41256
' BBS 12/17/2010 Changed database version from 79 to 80 for Ticket #41985
' BBS 12/21/2010 Change datagbase version from 80 to 81 for Ticket # 42352
' MM 1/4/2011 Changed database version from 81 to 82 - New table for pick expedite report
' BBS 1/17/2011 Changed database from Version 82 to 83 to add new fields for registration logic (Ticket #38600)
' BBS 1/26/2011 Changed database from version 83 to 84 for Ticket #44709


Public Const DB_VERSION = 84
...
Function OpenSqlServer(Optional bReStart As Boolean) As Boolean
   ...
   clsADOCon.Connect = "UID=" & sSaAdmin & ";PWD=" & sSaPassword & ";DRIVER={SQL Server};" _
                    & "SERVER=" & sserver & ";DATABASE=" & sDataBase & ";"
   clsADOCon.EstablishConnection rdDriverNoPrompt
   
   'clsadocon.Execute "alter table Version add TestDatabase tinyint not null default 0"
   'Err.Clear
...
'   Dim newver As Integer
   ...
   If bSqlRows Then
      ...
      ver = rdo!Version
   Else
      clsADOCon.Execute ("create table Version ( Version  int )")
      clsADOCon.Execute ("insert Version ( Version ) values ( 0 )")
      ver = 0
   End If
Debug.Print "Initial database version " & ver
   ...
   timeout = clsADOCon.QueryTimeout
   clsADOCon.QueryTimeout = 1200    '20 minutes
   ...
   If ver < 1 Then
      ...
      '        clsadocon.Execute "use msdb"
      '        clsadocon.Execute "ALTER TABLE dbo.SystemEvents ALTER COLUMN Event_Text varchar(4096)"
      '        clsadocon.Execute "use " & sDataBase
      
      'convert PsitTable.PISELLPRICE FROM real to decimal(12,4).  Remove default first
      clsADOCon.Execute "exec DropColumnDefault 'PsitTable', 'PISELLPRICE'"
      clsADOCon.Execute "alter table PsitTable alter column PISELLPRICE decimal(12,4) null"
      clsADOCon.Execute "alter table PsitTable add constraint PISELLPRICE_Default default ( 0.00 ) for PISELLPRICE"
      
      'basic security is no longer used
      clsADOCon.Execute "EXEC sp_rename 'UsscTable', 'UsscTable_Obsolete'"
      
      'set version = 1
      clsADOCon.Execute "update Version set Version = 1"
   End If
   ...
   If ver < 2 Then
      ...
      'set version
      clsADOCon.Execute "update Version set Version = 2"
   End If
   ...
   If ver < 3 Then
      ...
      'set version
      clsADOCon.Execute "update Version set Version = 3"
   End If
   
   If ver < 4 Then
      'fix primary key and index problems at Jevco China and other places
      'clsadocon.Execute "drop index Cihdtable.Cihd_Idx"
      DropIndex "Cihdtable", "Cihd_Idx"
      clsADOCon.Execute "alter table Cihdtable drop constraint PK_Cihdtable_CHECKNUMBER"
      clsADOCon.Execute "alter table Cihdtable WITH NOCHECK ADD CONSTRAINT PK_CihdTable PRIMARY KEY CLUSTERED(INVNO)"
      clsADOCon.Execute "create index InvCust ON Cihdtable(INVCUST)"
      clsADOCon.Execute "create index InvPref ON Cihdtable(INVPRE)"
      clsADOCon.Execute "create index InvSon ON Cihdtable(INVSO)"
      
      'set version
      clsADOCon.Execute "update Version set Version = 4"
   End If
...
'   End If
   ...
   If ver < 11 Then
      ...
      clsADOCon.Execute sSql, rdExecDirect
...
UpdateDatabase25
   ...
   clsADOCon.QueryTimeout = timeout
...
Private Function UpdateDatabase5()
    ...
    If ver < newver Then
        ver = newver
            ...
            If Not ColumnExists("Preferences", "HideModuleButton") Then
   ...
   sSql = sSql & "            AND ViewLohdPartTable.LOTNUMBER = LoitTable.LOINUMBER               " & vbCrLf _
    ...
    sSql = sSql & "         WHERE mrp_type IN (2, 3, 4, 11, 12, 17)             " & vbCrLf _
            ...
            sSql = " SELECT * FROM StatCdType " & vbCrLf _
                & " WHERE STATCODE_TYPE_REF = 'VE' AND STATCODE_TYPE_NAME = 'Vendor Record' " & vbCrLf _
                & " AND STATCODE_TYPE_UNIQUE_KEY = '1' " & vbCrLf _
...
Private Function UpdateDatabase9()
   ...
   If ver < newver Then
      ...
      sSql = "ALTER VIEW [dbo].[viewOpenAPTerms] " & vbCrLf _
        ...
        If StoredProcedureExists("RptRMFGoods") = False Then
      ...
      sSql = sSql & "   End " & vbCrLf
            ...
            Execute True, sSql
        End If  'If StoredProcedureExists
        'set version
        Execute False, "UPDATE Version set Version = " & newver
    End If
End Function
...
Private Function UpdateDatabase10()
   ...
   If ver < newver Then
       ver = newver
         ...
         bSqlRows = clsADOCon.GetDataSet(sSql, RdoMrpSeed, ES_FORWARD)
...
Private Function UpdateDatabase11()
   ...
   If ver < newver Then
       ver = newver
        ...
        If Not ColumnExists("EsReportUsers", "UserID") Then
            Execute False, "ALTER TABLE EsReportUserPermissions DROP CONSTRAINT FK_EsReportUserPermissions_EsReportUsers"
      ...
      'Ticket #32175 modifications (BBS 7/9/2010)
      If Not ColumnExists("RfqsTable", "RFQDATEAMPM") Then
        sSql = "ALTER TABLE [dbo].[RfqsTable] ADD " & vbCrLf
        sSql = sSql & " RFQDATEAMPM tinyint NULL, RFQDUEAMPM tinyint NULL"
        Execute False, sSql
      End If
      
      'Ticket #34761 modifications (BBS 7/9/2010)
      If Not ColumnExists("ComnTable", "CODONTALLOWMONOTPC") Then
        sSql = "ALTER TABLE [dbo].[ComnTable] ADD " & vbCrLf
        sSql = sSql & " CODONTALLOWMONOTPC tinyint NULL "
        Execute False, sSql
      End If
...
Private Function UpdateDatabase12()
   ...
   If ver < newver Then
      ...
      If Not ColumnExists("ComnTable", "DEFCOSTINGMETHOD") Then
        'Ticket #13951 modifications (BBS 7/21/2010)
        sSql = "ALTER TABLE [dbo].[ComnTable] ADD DEFCOSTINGMETHOD tinyint NULL"
        Execute False, sSql
      End If
...
Private Function UpdateDatabase13()
   ...
   If ver < newver Then
       ...
       'sConstNme = GetConstraintNme("SohdTable", "DF__SohdTable__SOREM")
       If NormalizeConstraintNme("SohdTable", "DF__SohdTable__SOREM", "DF_SohdTable_SOREMARKS") = True Then
       
            sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
            sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOREMARKS" & vbCrLf
            Execute False, sSql
        
            sSql = "ALTER TABLE dbo.SohdTable ALTER COLUMN SOREMARKS VARCHAR(MAX)" & vbCrLf
            Execute False, sSql
           
            sSql = "ALTER TABLE dbo.SohdTable ADD CONSTRAINT" & vbCrLf
            sSql = sSql & " DF_SohdTable_SOREMARKS DEFAULT ('') FOR SOREMARKS" & vbCrLf
            Execute False, sSql
   ...
   ' sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
   ' sSql = sSql & " DROP CONSTRAINT FK_SohdTable_CustTable" & vbCrLf
   ' Execute False, sSql
    
   ' sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
   ' sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SONUMBER" & vbCrLf
   ' Execute False, sSql
    
    'sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
    'sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOTYPE" & vbCrLf
    'Execute False, sSql
    
    'sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
    'sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOCUST" & vbCrLf
    'Execute False, sSql
    
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SODATE" & vbCrLf
'    Execute False, sSql
    
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOCYN" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOTAXABLE" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOSALESMAN" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOREP" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOPO" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOSTERMS" & vbCrLf
'    Execute False, sSql
    
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOTERMS" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOINVDASH" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SODAYS" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SONETDAYS" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOSTNAME" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOSTADR" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOCCONTACT" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOCPHONE" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOCEXT" & vbCrLf
'    Execute False, sSql
    
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOJOBNO" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SODIVISION" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOREGION" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOBUSUNIT" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOVIA" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOFOB" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SONAME" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOPREFIX" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOTAXEXEMPT" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOCREATED" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOREVISED" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOREMARKS" & vbCrLf
...
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOSHIPDATE" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOTEXT" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOFREIGHTDAYS" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOCANCELED" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOCFAX" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOCINTPHONE" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOCINTFAX" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOLOCKED" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable" & vbCrLf
'    sSql = sSql & " DROP CONSTRAINT DF_SohdTable_SOFROMBID" & vbCrLf
'    Execute False, sSql
'
'    sSql = "CREATE TABLE dbo.Tmp_SohdTable" & vbCrLf
'    sSql = sSql & " (" & vbCrLf
'    sSql = sSql & " SONUMBER int NULL," & vbCrLf
'    sSql = sSql & " SOTYPE char(1) NULL," & vbCrLf
'    sSql = sSql & " SOCUST char(10) NULL," & vbCrLf
'    sSql = sSql & " SODATE smalldatetime NULL," & vbCrLf
'    sSql = sSql & " SOCYN tinyint NULL," & vbCrLf
'    sSql = sSql & " SOCOMMISSION decimal(7, 2) NULL," & vbCrLf
'    sSql = sSql & " SOTAXABLE tinyint NULL," & vbCrLf
'    sSql = sSql & " SOSALESMAN char(4) NULL," & vbCrLf
'    sSql = sSql & " SOREP char(30) NULL," & vbCrLf
'    sSql = sSql & " SOPO char(20) NULL," & vbCrLf
'    sSql = sSql & " SOSTERMS char(2) NULL," & vbCrLf
'    sSql = sSql & " SOTERMS char(8) NULL," & vbCrLf
'    sSql = sSql & " SOINVDASH smallint NULL," & vbCrLf
'    sSql = sSql & " SOARDISC decimal(6, 3) NULL," & vbCrLf
'    sSql = sSql & " SODAYS smallint NULL," & vbCrLf
'    sSql = sSql & " SONETDAYS smallint NULL," & vbCrLf
'    sSql = sSql & " SOSTNAME char(40) NULL," & vbCrLf
'    sSql = sSql & " SOSTADR varchar(255) NULL," & vbCrLf
'    sSql = sSql & " SOCCONTACT char(20) NULL," & vbCrLf
'    sSql = sSql & " SOCPHONE char(12) NULL," & vbCrLf
'    sSql = sSql & " SOCEXT smallint NULL," & vbCrLf
'    sSql = sSql & " SOJOBNO char(6) NULL," & vbCrLf
'    sSql = sSql & " SODIVISION char(4) NULL," & vbCrLf
'    sSql = sSql & " SOREGION char(2) NULL," & vbCrLf
'    sSql = sSql & " SOBUSUNIT char(4) NULL," & vbCrLf
'    sSql = sSql & " SOVIA char(30) NULL," & vbCrLf
'    sSql = sSql & " SOFOB char(12) NULL," & vbCrLf
'    sSql = sSql & " SONAME char(20) NULL," & vbCrLf
'    sSql = sSql & " SOPREFDATE smalldatetime NULL," & vbCrLf
'    sSql = sSql & " SOPREFIX char(1) NULL," & vbCrLf
'    sSql = sSql & " SOCAP decimal(7, 2) NULL," & vbCrLf
'    sSql = sSql & " SOTAXEXEMPT char(30) NULL," & vbCrLf
'    sSql = sSql & " SODELDATE smalldatetime NULL," & vbCrLf
'    sSql = sSql & " SOCREATED smalldatetime NULL," & vbCrLf
'    sSql = sSql & " SOREVISED smalldatetime NULL," & vbCrLf
'    sSql = sSql & " SOREMARKS varchar(MAX) NULL," & vbCrLf
'    sSql = sSql & " SOSHIPDATE smalldatetime NULL," & vbCrLf
'    sSql = sSql & " SOTEXT char(6) NULL," & vbCrLf
'    sSql = sSql & " SOFREIGHTDAYS smallint NULL," & vbCrLf
'    sSql = sSql & " SOCANDATE smalldatetime NULL," & vbCrLf
'    sSql = sSql & " SOCANCELED tinyint NULL," & vbCrLf
'    sSql = sSql & " SOCFAX char(12) NULL," & vbCrLf
'    sSql = sSql & " SOCINTPHONE char(4) NULL," & vbCrLf
'    sSql = sSql & " SOCINTFAX char(12) NULL," & vbCrLf
'    sSql = sSql & " SOLOCKED tinyint NULL," & vbCrLf
'    sSql = sSql & " SOFROMBID int NULL," & vbCrLf
'    sSql = sSql & " SORD_pointer int NULL" & vbCrLf
'    sSql = sSql & " )  ON [PRIMARY]" & vbCrLf
'    sSql = sSql & "  TEXTIMAGE_ON [PRIMARY]" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SONUMBER DEFAULT (0) FOR SONUMBER" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOTYPE DEFAULT ('S') FOR SOTYPE" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOCUST DEFAULT ('') FOR SOCUST" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SODATE DEFAULT (getdate()) FOR SODATE" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOCYN DEFAULT (0) FOR SOCYN" & vbCrLf
'    Execute False, sSql
'
'    sSql = "EXECUTE sp_bindefault N'dbo.DEFZERO', N'dbo.Tmp_SohdTable.SOCOMMISSION'" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOTAXABLE DEFAULT (0) FOR SOTAXABLE" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOSALESMAN DEFAULT ('') FOR SOSALESMAN" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOREP DEFAULT ('') FOR SOREP" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOPO DEFAULT ('') FOR SOPO" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOSTERMS DEFAULT ('') FOR SOSTERMS" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOTERMS DEFAULT ('') FOR SOTERMS" & vbCrLf
'    Execute False, sSql
    
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOINVDASH DEFAULT (0) FOR SOINVDASH" & vbCrLf
'    Execute False, sSql
'
'    sSql = "EXECUTE sp_bindefault N'dbo.DEFZERO', N'dbo.Tmp_SohdTable.SOARDISC'" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SODAYS DEFAULT (0) FOR SODAYS" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SONETDAYS DEFAULT (0) FOR SONETDAYS" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOSTNAME DEFAULT ('') FOR SOSTNAME" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOSTADR DEFAULT ('') FOR SOSTADR" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOCCONTACT DEFAULT ('') FOR SOCCONTACT" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOCPHONE DEFAULT ('') FOR SOCPHONE" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOCEXT DEFAULT (0) FOR SOCEXT" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOJOBNO DEFAULT ('') FOR SOJOBNO" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SODIVISION DEFAULT ('') FOR SODIVISION" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOREGION DEFAULT ('') FOR SOREGION" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOBUSUNIT DEFAULT ('') FOR SOBUSUNIT" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOVIA DEFAULT ('') FOR SOVIA" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOFOB DEFAULT ('') FOR SOFOB" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SONAME DEFAULT ('') FOR SONAME" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOPREFIX DEFAULT ('') FOR SOPREFIX" & vbCrLf
'    Execute False, sSql
'
'    sSql = "EXECUTE sp_bindefault N'dbo.DEFZERO', N'dbo.Tmp_SohdTable.SOCAP'" & vbCrLf
'    Execute False, sSql
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOTAXEXEMPT DEFAULT ('') FOR SOTAXEXEMPT" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOCREATED DEFAULT (getdate()) FOR SOCREATED" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOREVISED DEFAULT (getdate()) FOR SOREVISED" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOREMARKS DEFAULT ('') FOR SOREMARKS" & vbCrLf
...
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOSHIPDATE DEFAULT (getdate()) FOR SOSHIPDATE" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOTEXT DEFAULT ('') FOR SOTEXT" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOFREIGHTDAYS DEFAULT (0) FOR SOFREIGHTDAYS" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOCANCELED DEFAULT (0) FOR SOCANCELED" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOCFAX DEFAULT ('') FOR SOCFAX" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOCINTPHONE DEFAULT ('') FOR SOCINTPHONE" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOCINTFAX DEFAULT ('') FOR SOCINTFAX" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOLOCKED DEFAULT (0) FOR SOLOCKED" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.Tmp_SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " DF_SohdTable_SOFROMBID DEFAULT (0) FOR SOFROMBID" & vbCrLf
'    Execute False, sSql
'
'    sSql = "IF EXISTS(SELECT * FROM dbo.SohdTable)" & vbCrLf
'    sSql = sSql & "  EXEC('INSERT INTO dbo.Tmp_SohdTable (SONUMBER, SOTYPE, SOCUST, SODATE, SOCYN, SOCOMMISSION, SOTAXABLE, SOSALESMAN, SOREP, SOPO, SOSTERMS, SOTERMS, SOINVDASH, SOARDISC, SODAYS, SONETDAYS, SOSTNAME, SOSTADR, SOCCONTACT, SOCPHONE, SOCEXT, SOJOBNO, SODIVISION, SOREGION, SOBUSUNIT, SOVIA, SOFOB, SONAME, SOPREFDATE, SOPREFIX, SOCAP, SOTAXEXEMPT, SODELDATE, SOCREATED, SOREVISED, SOREMARKS, SOSHIPDATE, SOTEXT, SOFREIGHTDAYS, SOCANDATE, SOCANCELED, SOCFAX, SOCINTPHONE, SOCINTFAX, SOLOCKED, SOFROMBID, SORD_pointer)" & vbCrLf
'    sSql = sSql & "     SELECT SONUMBER, SOTYPE, SOCUST, SODATE, SOCYN, SOCOMMISSION, SOTAXABLE, SOSALESMAN, SOREP, SOPO, SOSTERMS, SOTERMS, SOINVDASH, SOARDISC, SODAYS, SONETDAYS, SOSTNAME, SOSTADR, SOCCONTACT, SOCPHONE, SOCEXT, SOJOBNO, SODIVISION, SOREGION, SOBUSUNIT, SOVIA, SOFOB, SONAME, SOPREFDATE, SOPREFIX, SOCAP, SOTAXEXEMPT, SODELDATE, SOCREATED, SOREVISED, CONVERT(varchar(MAX), SOREMARKS), SOSHIPDATE, SOTEXT, SOFREIGHTDAYS, SOCANDATE, SOCANCELED, SOCFAX, SOCINTPHONE, SOCINTFAX, SOLOCKED, SOFROMBID, SORD_pointer FROM dbo.SohdTable WITH (HOLDLOCK TABLOCKX)')" & vbCrLf
'    Execute False, sSql
'
'    sSql = "DROP TABLE dbo.SohdTable" & vbCrLf
'    Execute False, sSql
'
'    sSql = "EXECUTE sp_rename N'dbo.Tmp_SohdTable', N'SohdTable', 'OBJECT' " & vbCrLf
'    Execute False, sSql
'
'    sSql = "CREATE UNIQUE CLUSTERED INDEX SoNumber ON dbo.SohdTable" & vbCrLf
'    sSql = sSql & " (" & vbCrLf
'    sSql = sSql & " SONUMBER" & vbCrLf
'    sSql = sSql & " ) WITH( PAD_INDEX = OFF, FILLFACTOR = 80, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]" & vbCrLf
'    Execute False, sSql
'
'    sSql = "CREATE NONCLUSTERED INDEX JobNumber ON dbo.SohdTable" & vbCrLf
'    sSql = sSql & " (" & vbCrLf
'    sSql = sSql & " SOJOBNO" & vbCrLf
'    sSql = sSql & " ) WITH( PAD_INDEX = OFF, FILLFACTOR = 80, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]" & vbCrLf
'    Execute False, sSql
'
'    sSql = "CREATE NONCLUSTERED INDEX SoCust ON dbo.SohdTable" & vbCrLf
'    sSql = sSql & " (" & vbCrLf
'    sSql = sSql & " SOCUST" & vbCrLf
'    sSql = sSql & " ) WITH( PAD_INDEX = OFF, FILLFACTOR = 80, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]" & vbCrLf
'    Execute False, sSql
'
'    sSql = "CREATE NONCLUSTERED INDEX SoPo ON dbo.SohdTable" & vbCrLf
'    sSql = sSql & " (" & vbCrLf
'    sSql = sSql & " SOPO" & vbCrLf
'    sSql = sSql & " ) WITH( PAD_INDEX = OFF, FILLFACTOR = 80, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]" & vbCrLf
'    Execute False, sSql
'
'    sSql = "CREATE NONCLUSTERED INDEX SoType ON dbo.SohdTable" & vbCrLf
'    sSql = sSql & " (" & vbCrLf
'    sSql = sSql & " SOTYPE" & vbCrLf
'    sSql = sSql & " ) WITH( PAD_INDEX = OFF, FILLFACTOR = 80, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]" & vbCrLf
'    Execute False, sSql
'
'    sSql = "ALTER TABLE dbo.SohdTable ADD CONSTRAINT" & vbCrLf
'    sSql = sSql & " FK_SohdTable_CustTable FOREIGN KEY" & vbCrLf
'    sSql = sSql & " (" & vbCrLf
'    sSql = sSql & " SOCUST" & vbCrLf
'    sSql = sSql & " ) REFERENCES dbo.CustTable" & vbCrLf
'    sSql = sSql & " (" & vbCrLf
'    sSql = sSql & " CUREF" & vbCrLf
'    sSql = sSql & " ) ON UPDATE  NO ACTION " & vbCrLf
'    sSql = sSql & "  ON DELETE  NO ACTION " & vbCrLf
'    sSql = sSql & " " & vbCrLf
'    Execute False, sSql
    
'    sSql = "CREATE TRIGGER DT_SohdTable ON dbo.SohdTable" & vbCrLf
'    sSql = sSql & "FOR  DELETE " & vbCrLf
'    sSql = sSql & "  AS " & vbCrLf
'    sSql = sSql & "  SAVE TRANSACTION SaveSoRows " & vbCrLf
'    sSql = sSql & "  Rollback TRANSACTION" & vbCrLf
'    Execute False, sSql
'
   End If
...
Private Function UpdateDatabase14()
   ...
   If ver < newver Then
      ...
      If Not ColumnExists("Preferences", "AutoSelectLastRun") Then
        'Ticket #13951 modifications (BBS 7/21/2010)
        sSql = "ALTER TABLE [dbo].[Preferences] ADD AutoSelectLastRun tinyint NULL"
        Execute False, sSql
      End If
      
      ' Alter the view to include LOTNUMBER
      ...
      sSql = sSql & "FROM dbo.InvaTable LEFT OUTER JOIN " & vbCrLf
      sSql = sSql & "    dbo.LohdTable ON dbo.InvaTable.INLOTNUMBER = dbo.LohdTable.LOTNUMBER AND dbo.InvaTable.INPART = dbo.LohdTable.LOTPARTREF" & vbCrLf
      sSql = sSql & "Where (dbo.InvaTable.INTYPE = 19) Or " & vbCrLf
      sSql = sSql & "     (dbo.InvaTable.INTYPE = 30)" & vbCrLf
      Execute False, sSql
      
      ' Alter the store procedure to include LOTNUMBER
      ...
      sSql = sSql & " " & vbCrLf
      sSql = sSql & "   -- select data for the report " & vbCrLf
      sSql = sSql & "   SELECT PARTNUM, LOTNUMBER, LOTUSERLOTID, INNO, PADESC, PAEXTDESC, INADATE, " & vbCrLf
      sSql = sSql & "         INAQTY,INAMT, TOTMATL,TOTLABOR,TOTEXP,TOTOH, " & vbCrLf
      sSql = sSql & "         CREDITACCT, DEBITACCT,flgMatl, flgLabor, " & vbCrLf
      sSql = sSql & "         flgExp , flgOH " & vbCrLf
      sSql = sSql & "      FROM #tempINVReport " & vbCrLf
...
Private Function UpdateDatabase15()
   ...
   If ver < newver Then
      ver = newver
      If Not ColumnExists("ComnTable", "ResaleNumber") Then
          sSql = "ALTER TABLE [dbo].[ComnTable] ADD ResaleNumber varchar(20) NULL"
          Execute False, sSql
      End If
    
      ' Alter the Excess Inventory report - store procedure
      sSql = "ALTER PROCEDURE [dbo].[InvMRPExcessReport]" & vbCrLf
...
'First lets find and drop all constraints related to the wip fields
    For i = 1 To 4
        ...
        If sConstraintName <> "" Then Execute False, "ALTER Table [dbo].[ComnTable] DROP CONSTRAINT " & sConstraintName
    Next i
    For i = 1 To 4
    ...
    If ColumnExists("ComnTable", "WIPCRLABACCT") Then Execute False, "Exec sp_rename 'ComnTable.WIPCRLABACCT', 'WIPLABACCT', 'COLUMN'"
    If ColumnExists("ComnTable", "WIPCRMATACCT") Then Execute False, "Exec sp_rename 'ComnTable.WIPCRMATACCT', 'WIPMATACCT', 'COLUMN'"
    If ColumnExists("ComnTable", "WIPCREXPACCT") Then Execute False, "Exec sp_rename 'ComnTable.WIPCREXPACCT', 'WIPEXPACCT', 'COLUMN'"
    If ColumnExists("ComnTable", "WIPCROHDACCT") Then Execute False, "Exec sp_rename 'ComnTable.WIPCROHDACCT', 'WIPOHDACCT', 'COLUMN'"
     
    'Now drop the other four unused columns
    If ColumnExists("ComnTable", "WIPDRLABACCT") Then Execute False, "ALTER TABLE [dbo].[ComnTable] DROP COLUMN WIPDRLABACCT"
    If ColumnExists("ComnTable", "WIPDRMATACCT") Then Execute False, "ALTER TABLE [dbo].[ComnTable] DROP COLUMN WIPDRMATACCT"
    If ColumnExists("ComnTable", "WIPDREXPACCT") Then Execute False, "ALTER TABLE [dbo].[ComnTable] DROP COLUMN WIPDREXPACCT"
    If ColumnExists("ComnTable", "WIPDROHDACCT") Then Execute False, "ALTER TABLE [dbo].[ComnTable] DROP COLUMN WIPDROHDACCT"
     
    'Last but not least, add the constraints back for the new fields with consistent meaningful names
    Execute False, "ALTER TABLE ComnTable ADD CONSTRAINT DF_ComnTable_WIPLABACCT DEFAULT ('') FOR WIPLABACCT"
    Execute False, "ALTER TABLE ComnTable ADD CONSTRAINT DF_ComnTable_WIPMATACCT DEFAULT ('') FOR WIPMATACCT"
    Execute False, "ALTER TABLE ComnTable ADD CONSTRAINT DF_ComnTable_WIPEXPACCT DEFAULT ('') FOR WIPEXPACCT"
    Execute False, "ALTER TABLE ComnTable ADD CONSTRAINT DF_ComnTable_WIPOHDACCT DEFAULT ('') FOR WIPOHDACCT"
...
Private Function UpdateDatabase17()
   ...
   If ver < newver Then
       ...
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.CihdTable') , 'InvNo' , 'IndexID' ) IS NULL CREATE INDEX InvNo ON CihdTable (INVNO)"
   
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.InvaTable') , 'INAQTY' , 'IndexID' ) IS NULL CREATE INDEX InaQty ON InvaTable (INAQTY)"
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.LoitTable') , 'LOIPSITEM' , 'IndexID' ) IS NULL CREATE INDEX LoiPSItem ON LoitTable (LOIPSITEM)"
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.LoitTable') , 'LoiQuantity' , 'IndexID' ) IS NULL CREATE INDEX LoiQuantity ON LoitTable (LOIQUANTITY)"
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.PartTable') , 'PaClass' , 'IndexID' ) IS NULL CREATE INDEX PaClass ON PartTable (PACLASS)"
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.PartTable') , 'PaProdCode' , 'IndexID' ) IS NULL CREATE INDEX  PaProdCode ON PartTable (PAPRODCODE)"
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.PshdTable') , 'PSNumber' , 'IndexID' ) IS NULL CREATE INDEX PSNumber ON PshdTable (PSNUMBER)"
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.PsitTable') , 'PISONumber' , 'IndexID' ) IS NULL CREATE INDEX PISONumber PsitTable ON (PISONUMBER)"
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.PsitTable') , 'PISORev' , 'IndexID' ) IS NULL CREATE INDEX PISORev ON PsitTable (PISOREV)"
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.PsitTable') , 'PIITNo' , 'IndexID' ) IS NULL CREATE INDEX PIITNo ON PsitTable (PIITNO)"
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.SohdTable') , 'SOSalesman' , 'IndexID' ) IS NULL CREATE INDEX SOSalesman ON SohdTable (SOSALESMAN)"
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.SoitTable') , 'ITSO' , 'IndexID' ) IS NULL CREATE INDEX ITSO ON SoitTable (ITSO)"
       Execute False, "IF INDEXPROPERTY ( OBJECT_ID('dbo.SoitTable') , 'ITRevAcct' , 'IndexID' ) IS NULL CREATE INDEX ITRevAcct ON SoitTable (ITREVACCT)"
...
Private Function UpdateDatabase19()
   ...
   If ver < newver Then
       ...
       Execute False, "ALTER TABLE CihdTable ALTER COLUMN INVSTADR VARCHAR(300) "
...
Private Function UpdateDatabase20()
   ...
   If ver < newver Then
       ver = newver
        If Not ColumnExists("EsReportCustSh01", "MOStartDte") Then Execute False, "ALTER TABLE EsReportCustSh01 ADD MOStartDte CHAR(8) NULL "
...
Private Function UpdateDatabase21()
   ...
   If ver < newver Then
        ver = newver
        If Not ColumnExists("PartTable", "PAINTERNALCMT") Then Execute False, "ALTER TABLE PartTable ADD PAINTERNALCMT VARCHAR(MAX) NULL "
...
Private Function UpdateDatabase22()
   ...
   If ver < newver Then
      ...
      If Not ColumnExists("Log_PartNo", "EsReportClosedRunsLog") Then Execute False, "ALTER TABLE EsReportClosedRunsLog ADD Log_PartNo CHAR(30) NULL "
      If Not ColumnExists("Log_RunNo", "EsReportClosedRunsLog") Then Execute False, "ALTER TABLE EsReportClosedRunsLog ADD Log_RunNo INT "
      If Not ColumnExists("Log_Closed", "EsReportClosedRunsLog") Then Execute False, "ALTER TABLE EsReportClosedRunsLog ADD Log_Closed TINYINT DEFAULT(0) "
      
      Execute False, "Update StCodeTable SET STATUS_REF = LTRIM(STATUS_REF) WHERE LEFT(STATUS_REF,1)=' '"
      ...
      Execute False, "ALTER Table dbo.StCmtTable DROP CONSTRAINT PK_StCmtTable_1"
      Execute False, "ALTER TABLE dbo.StCmtTable ALTER COLUMN STATUS_CMT_REF VARCHAR(30)NOT NULL"
...
Private Function UpdateDatabase24()
    ...
    If ver < newver Then
        ...
        If Not ColumnExists("ComnTable", "REGID") Then Execute False, "ALTER TABLE ComnTable ADD REGID INT NULL"
        If Not ColumnExists("ComnTable", "PRODUCTKEY") Then Execute False, "ALTER TABLE ComnTable ADD PRODUCTKEY varchar(25) NULL"
    ...
    If ver < newver Then
        ver = newver
         ...
         If Not ColumnExists("PartTable", "PAINTERNALCMT") Then
             Execute False, "ALTER TABLE PartTable ADD PAINTERNALCMT VARCHAR(4096) NULL"
         Else
             Execute False, "ALTER TABLE PartTable ALTER COLUMN PAINTERNALCMT VARCHAR(4096) NULL"
         End If
   ...
   If Err Then
      ...
      If DisplayError Or InStr(1, Err.Description, "timeout expired", vbTextCompare) > 0 Then
         ...
         If InStr(1, Err.Description, "timeout expired", vbTextCompare) > 0 Then
            msg = msg & clsADOCon.QueryTimeout & " second timeout occurred performing database update." & vbCrLf
         End If
...
Public Sub ClearResultSet(RdoDataSet As rdoResultset)
...
Sub GetCompany(Optional bWantAddress As Byte)
   ...
   Else
      sSql = "SELECT COREF,CONAME,COPHONE,COFAX,COGLVERIFY FROM ComnTable " _
             & "WHERE COREF=1"
   End If
   bSqlRows = clsADOCon.GetDataSet(ActRs, ES_STATIC)
   If bSqlRows Then
      With ActRs
         Co.Name = "" & Trim(!CONAME)
...
Public Function GetServerDate() As Variant
   ...
   sSql = "select getdate() AS ServerTime"
   If GetDataSet(sSql, RdoTme, ES_FORWARD) Then
...
Private Function NormalizeConstraintNme(sTableNme As String, sOldObjName As String, sNewObjName As String) As Boolean
    ...
    sOldNme = GetConstraintNme(sTableNme, "DF__SohdTable__SOREM")
    
    Execute False, "exec sp_rename '" & sOldNme & "','" & sNewObjName & "',OBJECT"
    NormalizeConstraintNme = True
    Exit Function
...
Public Function IsValidSQLDatabase(ByVal sDBName As String) As Boolean
    Dim RdoDB As rdoResultset
    On Error Resume Next
    sSql = "SELECT TOP 1 Name FROM sys.Databases WHERE Name='" & sDBName & "'"
    bSqlRows = clsADOCon.GetDataSet(sSql, RdoDB, ES_FORWARD)
    If bSqlRows Then IsValidSQLDatabase = True
    Set RdoDB = Nothing
End Function
